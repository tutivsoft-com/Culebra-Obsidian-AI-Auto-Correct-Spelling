...
- Free allowance: 2,000 characters granted once per install, tracked purely locally (`freeCredits` in plugin settings), never synced to or from Constance.
...
- Purchasing: the settings tab has three "Buy" buttons that open `GET https://app.tutivsoft.com/buy?app_id=culebra-ai-spell-correct&price_id=<tier>&email=<billing email>&external_customer_id=<installation id>` in the system browser via `window.open`.
- Identity: the user signs in or registers with an email and password; Constance links the persisted random `constanceDeviceId` installation id to the account. The password is used only for authentication and is never saved.
...
- Why the unsigned pattern: this plugin uses Constance's public browser-relay endpoints because it is a locally-installed client and cannot hold a server-side HMAC signing secret. These endpoints are reused unmodified from the Antero integration.
