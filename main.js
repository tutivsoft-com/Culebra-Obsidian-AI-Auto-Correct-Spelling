  ...
  * The local free-character pool is used first, followed by the Constance
  ...
  async correctEditorText(editor) {
    ...
    if (hasSelection ? editor.getSelection() !== originalText : editor.getValue() !== originalText) {
      new import_obsidian3.Notice("Culebra: the note changed while you reviewed it. Run the correction again to protect your edits.");
      return;
    }
  ...
  async correctFile(file) {
    ...
    if (await this.app.vault.read(file) !== originalText) {
      new import_obsidian3.Notice(`Culebra: ${file.name} changed while you reviewed it. Run the correction again to protect your edits.`);
      return;
    }
    if (!await this.chargeOneCredit(originalText.length)) return;
    if (await this.app.vault.read(file) !== originalText) {
