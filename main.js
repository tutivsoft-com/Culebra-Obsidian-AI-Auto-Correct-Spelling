...
var CulebraSpellCorrectPlugin = class extends import_obsidian3.Plugin {
  ...
  async openBuyCheckout(tier) {
    ...
    if (pending && (pending.planCode !== planCode || pending.priceId !== priceId)) {
      new import_obsidian3.Notice("Culebra: finish or retry the pending checkout before starting another purchase.", 5e3);
      return;
    }
    ...
    if (result.kind === "auth-required") {
      ...
      await this.saveSettings();
      new import_obsidian3.Notice("Culebra: your billing session expired. Sign in again before purchasing.", 6e3);
      return;
    }
    if (result.kind !== "ok") {
      new import_obsidian3.Notice(result.kind === "unavailable" ? `Culebra checkout unavailable (HTTP ${result.status}). Retry when Constance is reachable.` : "Culebra checkout could not be started. Retry when Constance is reachable.", 6e3);
      return;
    }
    ...
    new import_obsidian3.Notice("Opening secure Constance checkout...", 3e3);
...
var CorrectionPreviewModal = class extends import_obsidian3.Modal {
  constructor(app, targetLabel, originalText, correctedText) {
    super(app);
    __publicField(this, "targetLabel", targetLabel);
    __publicField(this, "originalText", originalText);
    __publicField(this, "correctedText", correctedText);
    __publicField(this, "decision");
    __publicField(this, "resolveDecision");
